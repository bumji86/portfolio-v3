...
const formats = {
  ...
  po: {
    codec: () => Promise.resolve().then(function () { return require('./BuiltInPoCodec-B6w-kmTx.cjs'); }),
    extension: '.po'
  }
};
...
async function resolveCodec(format, projectRoot) {
  if (isBuiltInFormat(format)) {
    ...
    const isPath = format.codec.startsWith('.') || path__default.default.isAbsolute(format.codec);
    let resolvedPath;
    if (isPath) {
      resolvedPath = path__default.default.resolve(projectRoot, format.codec);
    } else {
      try {
        resolvedPath = module$1.createRequire(path__default.default.join(projectRoot, 'noop.js')).resolve(format.codec);
      } catch (error) {
    ...
    let module;
    try {
      module = await import(url.pathToFileURL(resolvedPath).href);
    } catch (error) {
...
const require$2 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('plugin.cjs', document.baseURI).href)));
class MessageExtractor {
  ...
  async extract(absoluteFilePath, source) {
    ...
    const messages = JSON.parse(JSON.parse(output).results).filter(item => item.type === 'extracted').map(item => ({
      id: item.id,
...
function pruneUndefined(value) {
  ...
  if (value !== null && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).filter(([, entry]) => entry !== undefined).map(([key, entry]) => [key, pruneUndefined(entry)]));
  }
  return value;
}
...
function getNextConfig(pluginConfig, nextConfig, extractorConfig) {
  ...
  function getCatalogLoaderConfig() {
    ...
    const sourceLocale = messages.sourceLocale ?? (typeof experimental.extract === 'object' ? experimental.extract.sourceLocale : undefined);
    return {
